...
const currentUserId = 1; // docelowo: pobierz z auth
...
const reservationDays = computed(() => {
  if (reservationData.value.startDate && reservationData.value.endDate) {
    ...
    const ms = end - start;
    if (ms < 0) return 0;
    return Math.ceil(ms / (1000 * 60 * 60 * 24)) + 1; // +1 bo liczymy oba dni
  }
  return 0;
});

// --- Wyliczanie ceny: najpierw miesiące, potem tygodnie, reszta dni ---
const reservationCost = computed(() => {
  const days = reservationDays.value;
  if (!vehicle.value || days <= 0) return 0;
  let cost = 0;
  let left = days;

  if (vehicle.value.monthly_rate && left >= 30) {
    const months = Math.floor(left / 30);
    cost += months * vehicle.value.monthly_rate;
    left = left % 30;
  }
  if (vehicle.value.weekly_rate && left >= 7) {
    const weeks = Math.floor(left / 7);
    cost += weeks * vehicle.value.weekly_rate;
    left = left % 7;
  }
  if (vehicle.value.daily_rate && left > 0) {
    cost += left * vehicle.value.daily_rate;
  }
  return cost;
});
...
const toggleReservationForm = () => {
  ...
  // czyść form po zamknięciu
  if (!isReservationFormOpen.value) {
...
const handleReservation = async () => {
  ...
  try {
    const cost = reservationCost.value;
    const createRes = await ReservationService.createReservation({
      vehicle_id: vehicleId,
      client_id: currentUserId,
      start_date: reservationData.value.startDate,
      end_date: reservationData.value.endDate,
      cost,
      status: null
    });
    ...
    const carName = `${vehicle.value.make} ${vehicle.value.model}`;
    const paymentRes = await PaymentService.createCheckoutSession({
    ...
    alert('Błąd rezerwacji/płatności: ' + (err.response?.data?.error || err.message));
  }
};
          ...
          <div class="bg-white rounded-lg shadow-md overflow-hidden">
            ...
            <div class="p-6">
              <div class="flex justify-between items-start mb-4">
                ...
                <span
                    :class="[statusColor, 'px-3 py-1 text-sm font-semibold rounded-full']"
                >
                  {{ vehicle.status }}
                </span>
              </div>
            ...
            <div class="mb-6">
              <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-xl font-bold text-indigo-600">
                  {{ formatCurrency(vehicle.daily_rate) }} PLN
                </span>
              </div>
              <div class="flex justify-between items-center mb-2">
                <span class="text-gray-600">Weekly Rate:</span>
                <span class="text-lg font-semibold text-indigo-600">
                  {{ formatCurrency(vehicle.weekly_rate) }} PLN
                </span>
              </div>
              <div class="flex justify-between items-center">
                <span class="text-gray-600">Monthly Rate:</span>
                <span class="text-lg font-semibold text-indigo-600">
                  {{ formatCurrency(vehicle.monthly_rate) }} PLN
                </span>
              </div>
            </div>
      ...
      >
        ...
        <form @submit.prevent="handleReservation" class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
                type="date"
                :min="reservationData.startDate || today"
                id="endDate"
          ...
          <div class="md:col-span-2 flex flex-col mb-2">
            <div>
              <span class="font-semibold">Number of days:</span>
              {{ reservationDays }}
            </div>
            <div>
              <span class="font-semibold">Total cost:</span>
              {{ formatCurrency(reservationCost) }} PLN
            </div>
          </div>
